// M7 candidate for the exact wallet operation D = 8 * a * R.
//
...
// product.  Keep derivation.metal as the frozen M5 reference.
...
inline void fe_add(thread Fe &out, thread const Fe &left, thread const Fe &right) {
  ulong z[10];
  for (uint i = 0; i < 10; ++i) z[i] = ulong(left.v[i]) + ulong(right.v[i]);
...
inline bool fe_equal(thread const Fe &left_input, thread const Fe &right_input) {
  uchar left[32], right[32];
  fe_to_bytes(left, left_input);
  fe_to_bytes(right, right_input);
  uchar difference = 0;
  for (uint i = 0; i < 32; ++i) difference |= left[i] ^ right[i];
  return difference == 0u;
}

inline bool fe_is_zero(thread const Fe &input) {
  uchar bytes[32];
  fe_to_bytes(bytes, input);
  uchar aggregate = 0;
  for (uint i = 0; i < 32; ++i) aggregate |= bytes[i];
  return aggregate == 0u;
}
...
inline void fe_to_bytes(thread uchar *out, thread const Fe &input) {
  ...
  fe_reduce(value);
  uint h[10];
  for (uint i = 0; i < 10; ++i) h[i] = value.v[i];

  uint q = (h[0] + 19u) >> 26;
  q = (h[1] + q) >> 25; q = (h[2] + q) >> 26;
  q = (h[3] + q) >> 25; q = (h[4] + q) >> 26;
  q = (h[5] + q) >> 25; q = (h[6] + q) >> 26;
  q = (h[7] + q) >> 25; q = (h[8] + q) >> 26;
  q = (h[9] + q) >> 25;

  h[0] += 19u * q;
  h[1] += h[0] >> 26; h[0] &= (1u << 26) - 1u;
  h[2] += h[1] >> 25; h[1] &= (1u << 25) - 1u;
  h[3] += h[2] >> 26; h[2] &= (1u << 26) - 1u;
  h[4] += h[3] >> 25; h[3] &= (1u << 25) - 1u;
  h[5] += h[4] >> 26; h[4] &= (1u << 26) - 1u;
  h[6] += h[5] >> 25; h[5] &= (1u << 25) - 1u;
  h[7] += h[6] >> 26; h[6] &= (1u << 26) - 1u;
  h[8] += h[7] >> 25; h[7] &= (1u << 25) - 1u;
  h[9] += h[8] >> 26; h[8] &= (1u << 26) - 1u;
  h[9] &= (1u << 25) - 1u;

  out[0]=uchar(h[0]); out[1]=uchar(h[0]>>8); out[2]=uchar(h[0]>>16);
  out[3]=uchar((h[0]>>24)|(h[1]<<2)); out[4]=uchar(h[1]>>6);
  out[5]=uchar(h[1]>>14); out[6]=uchar((h[1]>>22)|(h[2]<<3));
  out[7]=uchar(h[2]>>5); out[8]=uchar(h[2]>>13);
  out[9]=uchar((h[2]>>21)|(h[3]<<5)); out[10]=uchar(h[3]>>3);
  out[11]=uchar(h[3]>>11); out[12]=uchar((h[3]>>19)|(h[4]<<6));
  out[13]=uchar(h[4]>>2); out[14]=uchar(h[4]>>10); out[15]=uchar(h[4]>>18);
  out[16]=uchar(h[5]); out[17]=uchar(h[5]>>8); out[18]=uchar(h[5]>>16);
  out[19]=uchar((h[5]>>24)|(h[6]<<1)); out[20]=uchar(h[6]>>7);
  out[21]=uchar(h[6]>>15); out[22]=uchar((h[6]>>23)|(h[7]<<3));
  out[23]=uchar(h[7]>>5); out[24]=uchar(h[7]>>13);
  out[25]=uchar((h[7]>>21)|(h[8]<<4)); out[26]=uchar(h[8]>>4);
  out[27]=uchar(h[8]>>12); out[28]=uchar((h[8]>>20)|(h[9]<<6));
  out[29]=uchar(h[9]>>2); out[30]=uchar(h[9]>>10); out[31]=uchar(h[9]>>18);
}
...
kernel void derivation_m7_radix2625_niels_addition_chain(
    device const uchar *view_scalar [[buffer(0)]],
    device const uchar *points [[buffer(1)]],
    device uchar *results [[buffer(2)]],
    device uchar *valid [[buffer(3)]],

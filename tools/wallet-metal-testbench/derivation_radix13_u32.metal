// M13 experimental candidate for the exact wallet operation D = 8 * a * R.
//
// It tests a pure-32-bit radix-2^13 field representation for Apple GPUs.
// Products fit in uint because 20 * (2^13-1)^2 < 2^31.  This removes all ulong
// arithmetic at the cost of more limbs and products.  M12 remains the safe
// fastest 25/26-bit reference until this candidate passes every byte check.
...
constant uint RADIX13_MASK = 8191u;

inline void fe_zero(thread Fe &out) {
  for (uint i = 0; i < 20; ++i) out.v[i] = 0u;
}
...
inline void fe_copy(thread Fe &out, thread const Fe &input) {
  for (uint i = 0; i < 20; ++i) out.v[i] = input.v[i];
}

// Reduce a 40-coefficient radix-2^13 polynomial modulo 2^255-19.
// B^20 = 2^260 = 608 (mod p).  Inputs are bounded so every accumulator stays
// below 2^32; the schedule and loop bounds are independent of secrets.
inline void fe_reduce_wide(thread Fe &out, thread uint *z) {
  for (uint i = 0; i < 39; ++i) {
    z[i + 1u] += z[i] >> 13;
    z[i] &= RADIX13_MASK;
  }
  ...
  // Keep the top limb to its actual eight bits.  Since
  // 256 * B^19 = 2^255 = 19 (mod p), top excess folds directly into limb 0.
  ...
  for (uint i = 0; i < 20; ++i) out.v[i] = z[i];
}

inline void fe_reduce(thread Fe &value) {
  uint z[40] = {0};
  for (uint i = 0; i < 20; ++i) z[i] = value.v[i];
  fe_reduce_wide(value, z);
}

inline void fe_add(thread Fe &out, thread const Fe &left, thread const Fe &right) {
  uint z[40] = {0};
  for (uint i = 0; i < 20; ++i) z[i] = left.v[i] + right.v[i];
  fe_reduce_wide(out, z);
}

inline uint radix13_modulus_limb(uint index) {
  if (index == 0u) return 8173u;
  if (index == 19u) return 255u;
  return 8191u;
}
...
inline void fe_sub(thread Fe &out, thread const Fe &left, thread const Fe &right) {
  uint z[40] = {0};
  for (uint i = 0; i < 20; ++i)
    z[i] = left.v[i] + 2u * radix13_modulus_limb(i) - right.v[i];
  fe_reduce_wide(out, z);
}
...
inline void fe_freeze(thread Fe &out, thread const Fe &input) {
  ...
  // Fold the five excess top bits using 2^255 == 19.
  for (uint pass = 0; pass < 3; ++pass) {
    const uint high = out.v[19] >> 8;
    out.v[19] &= 255u;
    out.v[0] += high * 19u;
    for (uint i = 0; i < 19; ++i) {
      out.v[i + 1u] += out.v[i] >> 13;
      out.v[i] &= RADIX13_MASK;
    }
  }
  ...
  int borrow = 0;
  for (uint i = 0; i < 20; ++i) {
    int digit = int(out.v[i]) - int(radix13_modulus_limb(i)) - borrow;
    const int next_borrow = int(digit < 0);
    digit += next_borrow * 8192;
    difference.v[i] = uint(digit);
    borrow = next_borrow;
  }
  const uint use_difference = uint(borrow ^ 1);
  const uint mask = 0u - use_difference;
  for (uint i = 0; i < 20; ++i)
    out.v[i] = (out.v[i] & ~mask) | (difference.v[i] & mask);
}
...
inline bool fe_equal(thread const Fe &left_input, thread const Fe &right_input) {
  ...
  uint difference = 0u;
  for (uint i = 0; i < 20; ++i) difference |= left.v[i] ^ right.v[i];
...
inline bool fe_is_zero(thread const Fe &input) {
  ...
  uint aggregate = 0u;
  for (uint i = 0; i < 20; ++i) aggregate |= value.v[i];
  return aggregate == 0u;
}
...
inline void fe_from_bytes(thread Fe &out, thread const uchar *bytes) {
  for (uint limb = 0; limb < 20; ++limb) {
    const uint bit = limb * 13u;
    const uint byte = bit >> 3;
    const uint shift = bit & 7u;
    uint word = uint(bytes[byte]);
    if (byte + 1u < 32u) word |= uint(bytes[byte + 1u]) << 8;
    if (byte + 2u < 32u) word |= uint(bytes[byte + 2u]) << 16;
    out.v[limb] = (word >> shift) & RADIX13_MASK;
  }
  out.v[19] &= 255u;
}
...
inline void fe_to_bytes(thread uchar *out, thread const Fe &input) {
  Fe value;
  fe_freeze(value, input);
  for (uint i = 0; i < 32; ++i) out[i] = 0u;
  for (uint limb = 0; limb < 20; ++limb) {
    const uint bit = limb * 13u;
    const uint byte = bit >> 3;
    const uint shift = bit & 7u;
    const uint word = value.v[limb] << shift;
    out[byte] |= uchar(word);
    if (byte + 1u < 32u) out[byte + 1u] |= uchar(word >> 8);
    if (byte + 2u < 32u) out[byte + 2u] |= uchar(word >> 16);
  }
}
...
kernel void derivation_m13_radix13_u32_niels(
    device const uchar *view_scalar [[buffer(0)]],
    device const uchar *points [[buffer(1)]],
    device uchar *results [[buffer(2)]],
    device uchar *valid [[buffer(3)]],

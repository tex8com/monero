// M11 candidate for the exact wallet operation D = 8 * a * R.
//
...
kernel void derivation_m11_projective_batchinvert(
    device const uchar *view_scalar [[buffer(0)]],
    device const uchar *points [[buffer(1)]],
...
// Pass 2: Montgomery batch inversion.  The first implementation is
// intentionally simple and auditable: one GPU thread performs the prefix and
// reverse chains.  It removes count expensive exponentiations at the cost of
// 3*count field multiplications.  Later stages may parallelize the scan.
kernel void derivation_m11_batch_inverse(
  ...
  Fe accumulator;
  fe_one(accumulator);
  for (uint record = 0; record < parameters.count; ++record) {
  ...
  uint remaining = parameters.count;
  while (remaining != 0u) {
...
kernel void derivation_m11_compress_batchinvert(
    device const uint *projective [[buffer(0)]],
